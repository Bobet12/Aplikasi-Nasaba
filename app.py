...
if not os.path.exists(EXCEL_FILE):
    df_init = pd.DataFrame(columns=["Tanggal", "Nama Nasabah", "Setoran Awal (Rp)", "Bunga Per Tahun (%)", "Tenor (Bulan)", "Total Akhir (Rp)"])
...
with st.sidebar:
    ...
    setoran = st.number_input("Setoran Awal (Rp)", min_value=0.0, step=100000.0, format="%.2f")
    bunga = st.number_input("Suku Bunga Per Tahun (%)", min_value=0.0, max_value=100.0, value=12.0, step=0.5)
    tenor = st.number_input("Tenor (Bulan)", min_value=1, max_value=360, value=12, step=1)
...
def hitung_bunga_majemuk(p, r_annual, n_months):
    ...
    for month in range(1, n_months + 1):
        ...
        saldo += bunga_bulan
        detail.append({"Bulan": month, "Bunga (Rp)": bunga_bulan, "Saldo (Rp)": saldo})
    return saldo, pd.DataFrame(detail)

# Fungsi Generator PDF
def generate_pdf(nama_nasabah, setoran_awal, rate, tenor_bulan, total_akhir, df_detail):
    pdf = FPDF()
    pdf.add_page()
...
if simpan_btn:
    if nama.strip() == "":
        ...
        df_updated = pd.concat([df_existing, new_data], ignore_index=False)
        ...
        # Download PDF
        pdf_bytes = generate_pdf(nama, setoran, bunga, tenor, total_akhir, df_detail)
        st.download_button(
